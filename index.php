...
<html>
  ...
  <body>
    ...
    <header>
      ...
      <h2>Room "<?= $_GET["room"] ?>" <a class="new-btn" href="<?= $root . (isset($_GET['type']) ? $_GET['type'] . ":" : "") ?>">New Room</a><?php if (!isset($_GET['type'])) { ?> <a class="new-btn" href="./string:<?= $_GET['room'] ?>">Send string</a><?php } ?></h2>
    </header>
    <div class="loader">
      <progress class="send" value="0" max="100"></progress>
      <progress class="recv" value="0" max="100"></progress>
      <div class="send-btn">
        <?php if (!isset($_GET['type']) || $_GET['type'] != 'string'): ?>
