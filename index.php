...
if ((!isset($_GET['room']) || empty($_GET['room'])) && !is_facebook()) {
    ...
    if (preg_match("/index.php/", $url)) {
        ...
        if ($type != null && !preg_match("|$type:|", $url)) {
            if (!preg_match("|/$|", $url)) {
                $url .= "/";
            }
            $url .= $type . ":";
...
<html>
  <head>
    <title>P2P File Share (<?= $room ?>)</title>
    ...
    <script>var room = '<?= $room ?>'; var transfer_type = '<?= $type ?>';</script>
  ...
  <body>
    ...
    <header>
      ...
      <h2>Room "<?= $room ?>" <a class="new-btn" href="<?= $root . $type ?>">New Room</a><?php if ($type == 'string') { ?> <a class="new-btn" href="./string:<?= $room ?>">Send string</a><?php } ?></h2>
    </header>
    <div class="loader">
